...
[IMAGE PROMPT]: A vertical flowchart diagram illustrating the Flang HLFIR-Aware Array Bounds Sanitizer compilation pipeline. At the top, a box labeled "Fortran Source Code (.f90)" has a downward arrow to "Flang Frontend (Parsing and Semantic Analysis)," which has a downward arrow to "HLFIR Construction." Below HLFIR Construction, a highlighted box labeled "HLFIR Instrumentation Pass (Bounds Check Insertion)" is drawn with a dashed border to indicate the new component, with a side annotation reading "Reads hlfir.declare descriptor metadata; instruments hlfir.designate operations." Below this, an arrow leads to "HLFIR-to-FIR Lowering," then to "FIR-to-LLVM IR Lowering," then to "LLVM Backend (Code Generation)," then to "Object Code and Linking." A separate box on the right labeled "Runtime Support Library (bounds-check.cpp)" is connected by an arrow from the "Object Code and Linking" box, with a label "Linked at link time." At the bottom, a box labeled "Executable with Bounds Checking" has two outward arrows: one labeled "Check passes: continue execution" and another labeled "Check fails: error diagnostic to stderr, program terminated." On the left side, a small box labeled "Flang Driver (flang-new)" has an arrow pointing into "HLFIR Instrumentation Pass" with the label "-fcheck=bounds enables pass." All boxes are rectangular with rounded corners, the new components are shaded in a distinct color, and arrows are labeled to indicate data or control flow.
...
[IMAGE PROMPT]: A horizontal data flow diagram showing the lifecycle of array descriptor metadata through the Flang compilation pipeline. On the far left, a box labeled "Fortran Source: real :: A(N, M); A(i, j) = ..." contains a code snippet. An arrow labeled "Parsing and Semantic Analysis" leads to a box labeled "AST with Symbol Table (variable name, source location, declared bounds)." An arrow labeled "HLFIR Construction" leads to a box containing two sub-boxes stacked vertically: the top sub-box is labeled "hlfir.declare @A : shape(N, M), lbounds(1, 1)" and the bottom sub-box is labeled "hlfir.designate @A[%i, %j] loc(file.f90:42)." An arrow labeled "HLFIR Instrumentation Pass" leads to an expanded box showing the designate operation now preceded by a check block: "if (%i < 1 or %i > N) call _FortranABoundsCheck(%i, 1, N, 1, 'A', 'file.f90', 42); if (%j < 1 or %j > M) call _FortranABoundsCheck(%j, 1, M, 2, 'A', 'file.f90', 42); hlfir.designate @A[%i, %j]." An arrow labeled "HLFIR-to-FIR Lowering" leads to a box labeled "fir.call @_FortranABoundsCheck(...)." An arrow labeled "FIR-to-LLVM Lowering" leads to a box labeled "LLVM IR: call @_FortranABoundsCheck(...)." A final arrow labeled "Runtime Execution" leads to a decision diamond labeled "index in bounds?" with two outgoing arrows: "Yes" leading to "Continue execution" and "No" leading to "Emit diagnostic and terminate." All elements are arranged left to right with clearly labeled arrows. Descriptor metadata fields (lower bound, upper bound, extent, stride, rank) are annotated in a legend box in the upper-right corner.
